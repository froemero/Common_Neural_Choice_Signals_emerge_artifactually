...
# ╔═╡ e83d6e48-dc5b-11ee-1eb9-33fb6db3e812
begin
	using Glob
	using DataFrames
	using JLD2
...
# ╔═╡ a159d7c8-d8bb-4c8c-95d4-95debf207a84
using CSV

# ╔═╡ 93837242-e6dc-4ca5-baf3-741a7d9125f0
begin
...
# ╔═╡ c9a5329b-06e5-402f-af97-3399a3813476
...
# ╔═╡ edf93fbc-ed9c-4c14-849f-8eae30d3d28e
begin
...
# ╔═╡ 875f8f45-6e49-47f9-80b0-2df3840bc314
chanIx = findfirst(lowercase.(pyconvert(Vector{String},eeg.ch_names)) .== "pz")

# ╔═╡ 2f950bd6-0158-4049-816b-9d49d54b0f61
sfreq = pyconvert(Float64,eeg.info["sfreq"]) 

# ╔═╡ c22b495b-d6f2-4fab-a619-5542c3bf2ea0
function fitModel_withrtsplit(events,eeg;epoched=false)
	#@show typeof(eeg),typeof(events),eeg[1:5],events[1,:]
	...
	if epoched
		...
		eeg_epoch = allowmissing(eeg_epoch)
		eeg_epoch[badIx] .= missing
		m = Unfold.fit(UnfoldModel,	
			Dict("Response"=>(respForm,times),
			     "Stimulus"=>(stimForm,times)), 	events,eeg_epoch)#solver=Unfold.solver_robust)

	else
		badIx = abs.(eeg).>250
	eeg = allowmissing(eeg)
	eeg[badIx] .= missing
		
 m  = Unfold.fit(UnfoldModel,
	Dict(
		"Stimulus"=>(stimForm,firbasis((-1,1),sfreq,"Stimulus")),
		"Response"=>(respForm,firbasis((-1,1),sfreq,"Response"))
	),events,eeg)#solver=(x,y)->Unfold.solver_krylov(x,y;GPU=true))
	end
	#@show typeof(m)
return m
end

# ╔═╡ 9bce6ffa-c313-4100-ac98-501fdede2d68
...
# ╔═╡ f86ef99c-e95c-41f1-9d18-4c0ef6ac5cd1
begin
	allEvents = []
	allData = []
	for sub in sublist[1:end]
		@show sub
		p = joinpath(basepath,sub)
			# get data
		#eeg = PyMNE.io.read_raw_eeglab(p;preload=false)
		#eeg.filter(0.1,nothing)
			# get events
		cnv(a::Float64) = Float64.(a)
		cnv(a) = NaN
		
		eeg = matread(joinpath(basepath,sub))["EEG"]
			
		
		a = eeg["event"]
		events_onesub =  DataFrame(
...
@show 
		...
		y = Array{Float32}(undef, Int(eeg["nbchan"]), Int(eeg["pnts"])); # create a container for reading data

		read!(joinpath(basepath,eeg["data"]), y) # rea
		push!(allData,deepcopy(y[chanIx:chanIx,:,:]))
	end
end



# ╔═╡ 4366230a-8763-41c9-b33c-09cb1050213b

dat = DataFrame(:subject=>sublist,:events =>allEvents,:eeg=>allData);

# ╔═╡ 1a0888f9-2dc1-4f78-9096-c881eb45c9c9
m_all = @by(dat,:subject,
		:deconv=[fitModel(:events[1],:eeg[1][1,:];epoched=false)]
)

# ╔═╡ 7e5437c4-7de5-465d-9135-a1372d05d46c
m_all_withrtsplit = @by(dat,:subject,
		:deconv=[fitModel_withrtsplit(:events[1],:eeg[1][1,:];epoched=false)]
)

# ╔═╡ 2be9a4fe-8c71-4ee8-8933-8ba74e39495d
CSV.write("2024-03-08_massunivariateresiduals.csv",c_all)

# ╔═╡ 550d1967-ed21-4357-8d81-5439079ac480
pwd()

# ╔═╡ 79b24ee7-b086-4ca9-b921-4632b4db1415
allEffects = combine(groupby(m_all_withrtsplit,[:subject]),
    :deconv=>(x->effects(Dict(:rt_split=>["slower=","faster"]),x[1]))=>AsTable);
	

# ╔═╡ 7591e854-0fba-4d35-b089-edd6b2710c04
begin
f = Figure(resolution=(1500,550))
#|>x->@subset(x,:basisname.!=="Stimulus")
h = data(@by(allEffects,[:rt_split,:time,:basisname],:yhat=mean(:yhat)))*mapping(:time,:yhat,color=:rt_split,col=:basisname)*visual(Lines)|>x->draw!(f,x; facet = (; linkxaxes = :none, linkyaxes = :all))
...
# ╔═╡ f554ca35-9595-4fda-a0b5-c62cce37408c
dat[s,:events]

# ╔═╡ 7e4f2697-68d4-4841-b3be-d02f76f247d8


# ╔═╡ fd21e5dd-21c2-412a-b4e7-7813a17d62db
md"""
## Following Code copied from UnfoldDecode.jl 
UnfoldDecode.jl is not yet a registered package and I did want to switch the whole package-manager to manual-mode to include a git
"""

# ╔═╡ 04a0581f-6202-4b33-b3a5-7c5c419b37d7
function singletrials(dat,uf_train::UnfoldLinearModelContinuousTime,
	eventTbl::DataFrame,target,eventcolumn)
	...
	startIX = Int.(floor.(onsets) .+ Int(round(-1 * sfreq)))
	stopIX = startIX .+ Int(round(1*sfreq)) .- 1
	
	
	X_train = modelmatrix(uf_train)

	out_e = Array{Float64}(undef,size(dat,1),stopIX[1]-startIX[1],length(onsets))#fromTo.step,length(fromTo))
...
# ╔═╡ 4f9f3ad2-0afd-40c5-afe8-56ac5bfc1ba3
begin
		allResidModels = []

for s = 1:size(m_all,1)

	#s = 2
	@info s
	uf=m_all[s,:deconv]

	#resid = dat[s,:eeg][1,1:size(modelmatrix(uf),1),1] .- modelmatrix(uf) * coef(uf)[1,:,:]
		resid_epoch = singletrials(dat[s,:eeg],uf,dat[s,:events],"Stimulus",:event)
			badIx = abs.(resid_epoch).>250
			resid_epoch = allowmissing(resid_epoch)
			resid_epoch[badIx] .= missing
		times = range(-1,1,length=size(resid_epoch,2))
			stimForm = @formula(0~1+rt_split)
			respForm = @formula(0~1+rt_split)#+mean*var)
		
			#eeg_epoch,times = Unfold.epoch(resid,dat[s,:events],(-1,1),sfreq)
		m = Unfold.fit(UnfoldModel,	
				Dict("Stimulus"=>(stimForm,times),"Response"=>(respForm,times)), 	dat[s,:events],resid_epoch)#solver=Unfold.solver_robust)
		
	push!(allResidModels,m)
end
end

# ╔═╡ f5caed78-0e50-434d-ba05-4def0e9a8df9
length(allResidModels)

# ╔═╡ f58d848b-1f92-4531-857e-b2bc8ffbaa62
begin
c_container = []
for s = 1:length(allResidModels)
	c = effects(Dict(:rt_split=>["slower=","faster"]),allResidModels[s])
	c.subject .=s
	push!(c_container,c)
end
c_all = reduce(vcat,c_container)
end

# ╔═╡ 8409f30c-059b-496c-bac7-4de73ef92ccd
let
f = Figure(size=(1000,450))

h = data(@by(c_all,[:rt_split,:time,:basisname],:yhat=mean(:yhat)))*mapping(:time,:yhat,color=:rt_split,col=:basisname)*visual(Lines)|>x->draw!(f,x)
...
# ╔═╡ 1f6be8e7-ad20-4379-ad01-0d0e4dfaadfe
let
f = Figure(size=(1000,450))

h = data(c_all)*mapping(:time,:yhat,color=:rt_split,col=:basisname,group=:subject=>nonnumeric)*visual(Lines)|>x->draw!(f,x)
xlims!(current_axis(),[-0.2,1])
ylims!(current_axis(),[-2,6.5])
	legend!(f[1,3],h)
f
...
# ╔═╡ 05fd073e-7787-4b44-ba54-3f00aea48962
...
manifest_format = "2.0"
project_hash = "ae5feca8fde5a5a4faae59708c8926363b4aeca1"
...
git-tree-sha1 = "223afe626c0cc901e4f6337973ad96119f889833"
...
[[deps.Unfold]]
deps = ["DSP", "DataFrames", "Distributions", "DocStringExtensions", "Effects", "FileIO", "GLM", "IterativeSolvers", "JLD2", "LinearAlgebra", "Logging", "MLBase", "Missings", "ProgressMeter", "Random", "SparseArrays", "StaticArrays", "Statistics", "StatsBase", "StatsFuns", "StatsModels", "Tables", "Test", "TimerOutputs", "Tullio"]
git-tree-sha1 = "62d17ffc411e3fb4f971524c59e3155b6b2f5819"
