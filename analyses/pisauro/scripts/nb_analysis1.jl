...
# ╔═╡ b0e721eb-14a6-4941-bfd3-f0bf4579a4f6
using Revise

# ╔═╡ 9ac95544-e75d-4a4b-b1e9-4d8934e3fd98
begin
	using Glob
	using MAT
	using DataFrames
...
# ╔═╡ 54596edc-33f2-4a39-ad1f-529b1f0eac0b
using AlgebraOfGraphics

# ╔═╡ f826e217-1368-4125-bd8e-b23dd38b4f71
using CairoMakie

# ╔═╡ f0fd5277-eee2-4680-af66-e8b2f92f795d
using UnfoldMakie

# ╔═╡ 4d89004e-08c2-4eb1-a255-a9c9a17b7abf
using MakieThemes

# ╔═╡ 2203a2a5-4701-4527-b1e2-fd4f05630b83
using TimerOutputs

# ╔═╡ 15a61046-b612-4732-91f7-5a5a64c950e5
md"""
## Introduction
This notebook replicates one finding from Pisauro 2016 nature communication (Figure 2a). We show that the finding is a spurious result from overlapping components and does not show evidence for response locked evidence accumulation
"""

# ╔═╡ 49a4f40e-4b29-4092-8a5b-f89f9cf73d6d
CairoMakie.activate!(type = "svg")

# ╔═╡ 8f789bce-56c2-4a07-99dd-94d927fafea0
set_theme!(MakieThemes.ggthemr(:fresh))

# ╔═╡ 14174583-b5e9-4576-8a63-becdcaa41d7a
md"""
## Data Loading
"""

# ╔═╡ 96180f79-fe66-48a8-9c5e-f1d344d31e26
basepath ="/store/data/pisauro_evidenceAccumulation_ds002734/ds002734/"

# ╔═╡ 79273ee2-ac07-47ee-aad4-18465bc29cbe
sublist = [d for d in readdir(basepath) if occursin("sub",d)]

# ╔═╡ 7129a03d-5d2e-42a5-8d0d-482acd72ccca
md"""
combine the data to one dataframe
"""

# ╔═╡ 457b9aab-f704-4f4a-981e-cb3788f843d0
md"""
## Data Fitting
Function to average data over channels and fit either a mass univariate (epoched=true) or a deconvution model (epoched=false)
"""

# ╔═╡ 8ef9f165-0199-44a9-bb5c-d273e0720534
function fitModel(events,eeg;epoched=false)
	@show typeof(eeg),typeof(events)
	eeg = mean(eeg,dims=1) # avg over channels
	if epoched
		eeg_epoch,times = Unfold.epoch(eeg,@subset(events,:type.=="response"),(-0.6,1),1000)
		m = Unfold.fit(UnfoldModel,	Dict(Any=>(@formula(0~1+rt_split),times)), 	@subset(events,:type.=="response"),eeg_epoch)
	else
 m  = Unfold.fit(UnfoldModel,
	Dict(
		"stimulus"=>(@formula(0~1),firbasis((-0.3,1),1000,"stimulus")),
		"response"=>(@formula(0~1+rt_split),firbasis((-0.6,1),1000,"response"))
	),events,eeg,eventcolumn="type")
	end
return m
end

# ╔═╡ d267b5b5-a00e-45fe-9262-52ea0abcce57
md"""
This channel selection is taken from the original study figure 1c. The electrode indices are calculated in the `nb_vis_electrode.jl` notebook.

!!! note
	the original study had per-subject ROIs which we dont have access to
"""

# ╔═╡ 42f3cc44-5445-4bdb-8ea6-02bad890d76b
chanIx = [63,19,26,38,44,8,27,28,37]

# ╔═╡ a134a33e-1445-439a-8d84-b89462f07683
md"""
fit models and combine the results
"""

# ╔═╡ c72e9d73-cb56-4502-949b-a7b9a2b80683
md"""
## Results
We use the effects function to easily calculate marginal outputs (=> recover the ERPs from the beta/slopes)
"""

# ╔═╡ 52bc8e0e-65f4-49ae-a3e5-c0ae10acf3c3
md"""
same plot as above, but with SEM attached - there is a plotting bug in UnfoldMakie, thus it looks a bit funny
"""

# ╔═╡ 60d5fdc8-d657-4d58-98cc-e18d545bcb9e
md"""
## Function to extract events
this function extracts the events from the mat files, combines them across sessions and returns nice dataframes
"""

# ╔═╡ 14305d43-bb9d-4510-bcf3-a4f77c0cec4e
begin
	function extract_events(events_onesub)
		d1 = extract_events_one(events_onesub["events"][1])
		d1.run .= 1
		d2 = extract_events_one(events_onesub["events"][2])
		d2.run .= 2
		return vcat(d1,d2)
	end
		function extract_events_one(e)
			
	lat_s = e["tstim"][1,:]#.*5
	lat_r = e["respt"][1,:]#.*5
	rt = e["rt"][1,:]
	
	d_resp = DataFrame(:latency => lat_r,:type=>"response",:rt=>rt) # to get to FS = 5000
	d_stim = DataFrame(:latency => lat_s,:type=>"stimulus",:rt=>rt) # to get to FS = 
	5000

	
	d = vcat(d_resp,d_stim)

	d.rt_split = CategoricalArray(["slower=","faster"][1 .+ (d.rt .<= median(d.rt))])
	sort!(d,:latency)
	return d
	end
end

# ╔═╡ ae8f170d-cba8-4e6e-afc9-d6c8c8be07d3
...
begin
	...
	for sub in sublist
		@show sub
#sub = sublist[1]
	sub_no_minus = replace(sub,"-"=>"")
	eventpath = joinpath(basepath,sub,"EEG","EEG_events_$(sub_no_minus).mat")
	events_onesub = extract_events(matread(eventpath))
	
	function get_data_onerun(sub_no_minus,r)
		onerunpath = joinpath(basepath,sub,"EEG","EEG_data_$(sub_no_minus)_run_0$(r).mat")
		vars = matread(onerunpath)
		return vars["Y"]
	end
	data_run1 = get_data_onerun(sub_no_minus,1)
	data_run2 = get_data_onerun(sub_no_minus,2)
	eeg = hcat(data_run1,data_run2)
	events_onesub.latency[events_onesub.run .== 2] .+= size(data_run1,2)
...
# ╔═╡ 15360daf-bc4d-4a8b-b2df-7babf6c06e72
dat = DataFrame(:subject=>sublist,:events =>allEvents,:eeg=>allData);

# ╔═╡ 5e9cf60d-8726-4177-9729-10c845ebe973
m_all = @by(dat,:subject,
		:deconv=[fitModel(:events[1],:eeg[1][chanIx,:];epoched=false)],
		:epoched=[fitModel(:events[1],:eeg[1][chanIx,:];epoched=true)]
)

# ╔═╡ 94f91e06-95d3-410d-8998-98abba980359
m_all_stack  =stack(m_all,[:epoched,:deconv];variable_name="type",value_name="model");

# ╔═╡ 70ef9b84-78e8-49dd-8286-6934498e4eec
allEffects = combine(groupby(m_all_stack,[:subject,:type]),
    :model=>(x->effects(Dict(:rt_split=>["slower=","faster"]),x[1]))=>AsTable);
	

# ╔═╡ c2484da9-7877-49e8-81d3-6e46e5e83cc6
begin
...
# ╔═╡ 37290ff8-c942-44ba-9d82-769c2521f8c1
begin
	using CSVFiles
	save(joinpath(basepath,"allEffects_channelCluster.csv"),allEffects)
end

# ╔═╡ b8571013-9ea5-4d73-8416-77e2926dc76d
plot_erp(@by(allEffects,[:type,:rt_split,:time,:basisname],
		:yhat=mean(:yhat),
		:se_low=mean(:yhat).-std(:yhat)./sqrt(21),
		:se_high=mean(:yhat).+std(:yhat)./sqrt(21))|>x->
											@subset(x,:basisname.!=="stimulus"),
	mapping=(;color=:rt_split,col=:type,group=:type),extra=(;stderror=true))

# ╔═╡ 93fdf3f2-b359-43a4-a237-9e3ab97b31bb
plot_erp(@by(allEffects,[:type,:rt_split,:time,:basisname],
		:yhat=mean(:yhat),
		:se_low=mean(:yhat).-std(:yhat)./sqrt(21),
		:se_high=mean(:yhat).+std(:yhat)./sqrt(21))|>x->
...
# ╔═╡ 6d62f50c-0e66-4bc5-b2a7-a28dec12143e
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "704f2ec6fcb3e4ed68231a63ddf7ea9478b34598"
...
[[deps.BSplineKit]]
deps = ["ArrayLayouts", "BandedMatrices", "FastGaussQuadrature", "LazyArrays", "LinearAlgebra", "PrecompileTools", "Random", "Reexport", "SparseArrays", "Static", "StaticArrays", "StaticArraysCore"]
git-tree-sha1 = "d97597118325153436a52607c3399bf1164e3c48"
...
git-tree-sha1 = "97f5b18c92d58dac051d4c4657493e5032822f1d"
...
[[deps.Unfold]]
deps = ["BSplineKit", "CategoricalArrays", "DSP", "DataFrames", "Distributions", "DocStringExtensions", "Effects", "FileIO", "GLM", "IterativeSolvers", "JLD2", "LinearAlgebra", "Logging", "MLBase", "Missings", "MixedModels", "PkgBenchmark", "ProgressMeter", "Random", "RobustModels", "SparseArrays", "StaticArrays", "Statistics", "StatsBase", "StatsFuns", "StatsModels", "Tables", "Test", "TimerOutputs"]
git-tree-sha1 = "6cd940fced2aa4e94ab9b803cd3d7bc87ce5479d"
uuid = "181c99d8-e21b-4ff3-b70b-c233eddec679"
version = "0.5.1"

[[deps.UnfoldMakie]]
deps = ["AlgebraOfGraphics", "CategoricalArrays", "ColorSchemes", "ColorTypes", "Colors", "DataFrames", "DataStructures", "GeometryBasics", "GridLayoutBase", "ImageFiltering", "LinearAlgebra", "Makie", "SparseArrays", "Statistics", "TopoPlots", "Unfold"]
git-tree-sha1 = "2a7022987393a5bdcce12993a34cac6bd61c7dcf"
