...
allSubDataTablePC.Appraisal(allSubDataTable.SubNum~=5028 & allSubDataTable.Choice1~=-1,:) = scoretable.Valuation;
allSubDataTablePC.Choice(allSubDataTable.SubNum~=5028 & allSubDataTable.Choice1~=-1,:) = scoretable.Choice;
% exclude subject that's missing SUBEVALs
allSubDataTablePC = allSubDataTablePC(allSubDataTablePC.SubNum~=5028,:);
...
for Subject = 1:length(vps)
    ...
    %% load one dataset
    EEG = pop_loadset('filename',sprintf('%s_cleanNF.set',vpn),'filepath',sprintf('%sEEG_clean/',PATH));
    ...
    %%
    for IVs = 1: length(SubData(:,[7:13 18 19 44:45]).Properties.VariableNames)
        cnt=0;

        currname= SubData(:,[7:13 18 19 44:45]).Properties.VariableNames{IVs};
            ...
            if strcmp(EEG.event(iii).type, 'S 10')
                if iii<discarter
                    ...
                    if strcmp(EEG.event(iii+1).type, 'S 20')
                        ...
                        % here get the triger timing fixed - in
                        % Epoching:
                        OGLatency = EEG.event(iii+1).latency;
                        EEG.event(iii+1).latency =  EEG.event(iii).latency + round((SubData.RTeval1(cnt)*1000)/2);
                        CompOldNew= EEG.event(iii+1).latency-OGLatency;

                        fprintf('Fixing Trial %d RT latency from %d to %d, with difference %d \n ', cnt, OGLatency, EEG.event(iii+1).latency, CompOldNew)
                    end
                end
    ...
    cfgDesign.formula = {'y ~ 1+Appraisal+ Choice','y~1+Appraisal+ Choice'};
    ...
    save(sprintf('~/Dropbox (Brown)/CLPS-ShenhavLab/EEG_Studies/Experiments/BASB_EEG/Analyses/Matlab/Unfold_Analyses/UF_Results/%s_ufresult.mat', vpn), 'ufresult', '-v7.3');
    %display(ufresult)
