...
% load data
PATH = '~/Dropbox (Brown)/CLPS-ShenhavLab/EEG_Studies/Experiments/BASB_EEG/Analyses/Matlab/Unfold_Analyses_BASS/';
...
SOURCEFILES = dir(strcat(PATH, 'UF_Results/*.mat')); %all cleaned files
SUBJECTS = 1:numel(SOURCEFILES);
clear 'sbasicCoeffs'
clear 'rbasicCoeffs'
clear 'prbasicCoeffs'

sbasicCoeffs.labels={'intercept', 'PCAppraisal', 'PCChoiceDifficulty'};
rbasicCoeffs.labels={'intercept', 'PCAppraisal', 'PCChoiceDifficulty'};
prbasicCoeffs.labels={'intercept','PCAppraisal', 'PCChoiceDifficulty'};

sbasicCoeffs.times= ufresult.times([find(ufresult.times== -0.2) : find(ufresult.times== 1.5)]);
rbasicCoeffs.times= ufresult.times([find(ufresult.times== -1.5) : find(ufresult.times== 0.2)]);
prbasicCoeffs.times=ufresult.times([find(ufresult.times== -0.2) : find(ufresult.times== 1)]);

for subj = 1:length(SUBJECTS)
    
    % load dataset
    load(sprintf('%sUF_Results/%s',PATH, SOURCEFILES(subj).name))
    
    % concatenate datasets and save them as separate structures for s, r,
    % and post r
%     sbasicCoeffs.data(:,:,:,subj)=ufresult.beta(:,[find(ufresult.times== -0.2) : find(ufresult.times== 1.5)], 1:3);
%     
%     rbasicCoeffs.data(:,:,:,subj)=ufresult.beta(:,[find(ufresult.times== -1.5) : find(ufresult.times== 0.2)], 5:7);
%     
%     prbasicCoeffs.data(:,:,:,subj)=ufresult.beta(:,[find(ufresult.times== -0.2) : find(ufresult.times== 1)], 5:7);
%     

    sbasicCoeffs.data(:,:,:,subj)=ufresult.beta(:,[find(ufresult.times== -0.2) : find(ufresult.times== 1.5)], 1:3);
    
    rbasicCoeffs.data(:,:,:,subj)=ufresult.beta(:,[find(ufresult.times== -1.5) : find(ufresult.times== 0.2)], 4:6);
    
    prbasicCoeffs.data(:,:,:,subj)=ufresult.beta(:,[find(ufresult.times== -0.2) : find(ufresult.times== 1)], 4:6);
